                            ...
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    @if ($event->actual_participants >= $event->max_participants)
                                        <button disabled
                                            class="bg-gray-300 text-white px-4 py-2 rounded cursor-not-allowed">Full</button>
                                    @elseif (\App\Models\ScoreClaim::where('user_id', auth()->id())->where('event_id', $event->id)->exists())
                                        <button disabled
                                            class="bg-gray-300 text-white px-4 py-2 rounded cursor-not-allowed">Registered</button>
                                    @else
